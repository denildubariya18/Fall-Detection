...
while True:
    ...
    for r in results:
        ...
        for box in boxes:
            ...
            if cls == 0 and conf > 0.3 and w>h:
                ...
                cvzone.putTextRect(img, f'Fall Detected {conf}', (max(0, x1), max(35, y1)), scale=1, thickness=1, offset=3)
    ...
    if fall_detected and fall_start_time is not None and (time.time()-fall_start_time)>10 and mailed is False and temp!=listSize:
        cv2.imwrite('Fall-detection\\fallImage.jpg',img)
        send_email('Fall-detection\\fallImage.jpg')
        temp=listSize
        mailed=True
